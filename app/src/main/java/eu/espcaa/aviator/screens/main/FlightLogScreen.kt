...
fun FlightLogScreen(
    ...
    val airlineCode = remember { mutableStateOf("") }
    val airlineName = remember { mutableStateOf("") }
    val errorText = remember { mutableStateOf("") }
    val departureAirportCode = remember { mutableStateOf("") }
    val arrivalAirportCode = remember { mutableStateOf("") }
    ...
    Box(
        ...
        Column {
            ...
            if (flightViewModel.flights.isEmpty()) {
                ...
                Column(
                    modifier = Modifier.padding(16.dp).verticalScroll(scrollState).padding(bottom = 112.dp)
                ) {
                        ...
                        Card (
                            modifier = Modifier
                                ...
                                .pointerInput(Unit) {
                                    detectTapGestures(
                                        onLongPress = {
                                            activeFlightId.value = flight.flightId
                            ...
                            Row(
                                ...
                                Log.d("FlightLogScreen", "Image URL: ${BuildConfig.BASE_URL}api/logo/getLogo?icao=${flight.airlineCode}")
                                Box (
                                    modifier = Modifier
                                        .size(50.dp)
                                    ...
                                    AsyncImage(
                                        model = ImageRequest.Builder(LocalContext.current)
                                            .data("${BuildConfig.BASE_URL}api/logo/getLogo?icao=${flight.airlineCode}")
                                            .crossfade(true)
                                            .build(),
    ...
    if (showDetailsBottomSheet.value) {
        ...
        ModalBottomSheet(
            ...
            val flight = flightViewModel.flights.firstOrNull { it.flightId == activeFlightId.value }
            if (flight == null) {
                Text(
                ...
                Column(
                    ...
                    Row(
                        ...
                        Text(
                            text = flight.airlineCode + " - " + flight.departureCode + " to " + flight.arrivalCode,
                            style = MaterialTheme.typography.headlineSmall,
                            color = MaterialTheme.colorScheme.primary,
                            modifier = Modifier.padding(start = 16.dp)
                        )
            ...
            if (bottomSheetLoading.value) {
                ...
                Row(
                    ...
                    OutlinedTextField(
                        ...
                        onValueChange = {
                            if (it.length < 3) {
                                airlineCode.value = it.uppercase(Locale.getDefault())
                            } else {
                                airlineCode.value =
                    ...
                    IconButton(
                        onClick = {
                            navController.navigate("search?type=airline")
                        },
                ...
                Row(
                    ...
                    OutlinedTextField(
                        value = departureAirportCode.value,
                        onValueChange = {
                            if (it.length < 3) {
                                departureAirportCode.value = it.uppercase(Locale.getDefault())
                            } else {
                                departureAirportCode.value = it.substring(0, 3).uppercase(Locale.getDefault())
                            }
                        },
                    ...
                    IconButton(
                        onClick = {
                            navController.navigate("search?type=airport")
                        },
                        colors = IconButtonDefaults.iconButtonColors(
                            containerColor = MaterialTheme.colorScheme.primary,
                            contentColor = MaterialTheme.colorScheme.onPrimary,
                ...
                Row(
                    ...
                    OutlinedTextField(
                        value = arrivalAirportCode.value,
                        onValueChange = {
                            if (it.length < 3) {
                                arrivalAirportCode.value = it.uppercase(Locale.getDefault())
                            } else {
                ...
                OutlinedButton(
                    ...
                    Text(
                        text = if (selectedDate.value.isEmpty()) "Select a date" else selectedDate.value,
                ...
                Button(
                    onClick = {
                        if (arrivalAirportCode.value.isEmpty() || airlineCode.value.isEmpty() || departureAirportCode.value.isEmpty()) {
                            errorText.value = "Please fill all of the fields with a correct value."
                        } else if (arrivalAirportCode.value.length < 3 || airlineCode.value.length < 3 || departureAirportCode.value.length < 3) {
                            errorText.value = "Please ensure all codes are at least 3 characters long."
                        } else if (selectedDate.value.isEmpty()) {
                            errorText.value = "Please select a date for your flight."
                        } else if (departureAirportCode.value == arrivalAirportCode.value) {
                            errorText.value = "Departure and arrival airport codes cannot be the same."
                        }
                            ...
                            scope.launch {
                                if (authState is AuthState.Authenticated) {
                                    ...
                                    try {
                                        val response = ApiClient.createFlightApi.createFlight(
                                            CreateFlightRequest(
                                                sessionToken = token,
                                                airlineCode = airlineCode.value,
                                                arrivalCode = arrivalAirportCode.value,
                                                departureCode = departureAirportCode.value,
                                                departureDate = selectedDate.value,
                                            )
                                        )

                                        if (response.success) {
                                            val currentAirlineCode = airlineCode.value
                                            val currentDepartureCode = departureAirportCode.value
                                            val currentArrivalCode = arrivalAirportCode.value
                                            val currentDepartureDate = selectedDate.value
                                            ...
                                            flightViewModel.addFlight(
                                                Flight(
                                                    ...
                                                    departureCode = currentDepartureCode,
                                                    arrivalCode = currentArrivalCode,
                                                    departureDate = currentDepartureDate,
                                                    departureAirportCoords = Pair(response.positions.departure.lat, response.positions.departure.lon) as Pair<Double, Double>,
                                                    arrivalAirportCoords = Pair(response.positions.arrival.lat, response.positions.arrival.lon) as Pair<Double, Double>,
                                        ...
                                        errorText.value = "Server error: HTTP ${e.code()}\n$errorBody"
                                        bottomSheetLoading.value = false
                                    } catch (e: Exception) {
                                        // Network or unexpected error
                                        errorText.value = "Unexpected error: ${e.localizedMessage}"
                                        bottomSheetLoading.value = false
                                    }
                                } else {
            ...
            confirmButton = {
                Button(
                    onClick = {
                        ...
                        selectedDate.value = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date(millis))
